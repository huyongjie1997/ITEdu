...
from .models import CourseInfo,LessonInfo,SourceInfo,VideoInfo
...
def course_list(request):
    ...
    #按序号的先后做经典课程推荐
    recommend_courses = all_courses.order_by('id')[:2]
...
def course_detail(request,course_id):
    if course_id:
        ...
        #按学习人数最多的做相关课程推荐
        relate_courses = CourseInfo.objects.filter(category=course.category).exclude(id=int(course_id)).order_by('-study_num')[:1]
...
def course_video(request,course_id):
    if course_id:
        ...
        usercourse_list = UserCourse.objects.filter(study_course=course)[:1]
        ...
        usercourse_list = UserCourse.objects.filter(study_man__in=user_list).exclude(study_course=course).order_by('-add_time')[:2]
...
def course_play(request,course_id,video_id):
    if course_id:
        if video_id:
            course = CourseInfo.objects.filter(id = int(course_id))[0]
            video = VideoInfo.objects.filter(id = int(video_id))[0]
            # 学过该课的同学还学过什么课程
            # 第一步：我们需要从中间表用户课程表当中找到学过该课的所有对象
            usercourse_list = UserCourse.objects.filter(study_course=course)[:1]
